...
function checkAdminAccessAttempts($max_attempts = 5){
    global $CMSNT;
    $ip_address = myip();
    if($max_attempts == 5){
...
function CMSNT_check_license($licensekey, $localkey='') {
    // Trả về kết quả giả lập rằng giấy phép luôn hợp lệ
    $results = array(
        'status' => 'Active',
        'description' => 'Giấy phép hợp lệ (bỏ qua kiểm tra)',
        'checkdate' => date("Ymd"),
        'localkey' => '' // Có thể để trống hoặc tạo một giá trị giả
    );
    return $results;
}
...
function checkLicenseKey($licensekey){
    ...
    $results = array(
        'status' => true,
        'msg' => 'Giấy phép hợp lệ (bỏ qua kiểm tra)'
    );
    return $results;
}
